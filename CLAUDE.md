...
**ai-slot-component**：存量页面的 AI 内容交付层。页面作者把原有 HTML 包进 `<ai-slot>` 自定义元素，AI 侧返回**结构化组件树 JSON**（绝不返回 HTML），经校验后由各框架适配器渲染为真实组件；失败/超时/校验不过则静默回退到元素内的原始兜底内容。定位是「AI 内容的安全上线通道」：构建期预生成（生产环境零运行时 LLM 调用）、失效推送让内容更新免重新部署、可接 A2UI/agent 端点；访客个性化（`editable`）是特性之一，不是主叙事。当前 v1 + v1.1 已全部合入 master。
